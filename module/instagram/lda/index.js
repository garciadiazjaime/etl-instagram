...
//
let process = function (sentences, numberOfTopics, numberOfTermsPerTopic, languages, alphaValue, betaValue, randomSeed) {
  ...
  if (sentences && sentences.length > 0) {
    ...
    languages.forEach((value) => {
      let stopwordsLang = require(`./stopwords_${  value  }.js`);
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      let words = sentences[i] ? sentences[i].split(/[\s,\"]+/) : null;
    ...
    let V = vocab.length;
    let M = documents.length;
    let K = parseInt(numberOfTopics);
    let alpha = alphaValue || 0.1; // per-document distributions over topics
    let beta = betaValue || 0.01; // per-topic distributions over words
    documents = documents.filter((doc) => doc.length); // filter empty documents

    lda.configure(documents, V, 10000, 2000, 100, 10, randomSeed);
    lda.gibbs(K, alpha, beta);

    let theta = lda.getTheta();
    let phi = lda.getPhi();
    ...
    for (let k = 0; k < phi.length; k++) {
      ...
      for (let t = 0; t < topTerms; t++) {
        let topicTerm = things[t].split('_')[2];
        let prob = parseInt(things[t].split('_')[0] * 100);
        if (prob < 2) continue;
...
var lda = new function () {
  let documents; var z; var nw; var nd; var nwsum; var ndsum; var thetasum; var phisum; var V; var K; var alpha; var 
 beta;
  let THIN_INTERVAL = 20;
  let BURN_IN = 100;
  let ITERATIONS = 1000;
  let SAMPLE_LAG;
  let RANDOM_SEED;
  ...
  this.initialState = function (K) {
    ...
    for (let m = 0; m < M; m++) {
      let N = this.documents[m].length;
      this.z[m] = new Array();
      for (let n = 0; n < N; n++) {
        let topic = parseInt('' + (this.getRandom() * K));
        this.z[m][n] = topic;
    ...
    for (i = 0; i < this.ITERATIONS; i++) {
      for (let m = 0; m < this.z.length; m++) {
        for (let n = 0; n < this.z[m].length; n++) {
          let topic = this.sampleFullConditional(m, n);
          this.z[m][n] = topic;
        }
      }
  ...
  this.sampleFullConditional = function (m, n) {
    ...
    let u = this.getRandom() * p[this.K - 1];
  ...
  this.getTheta = function () {
    let theta = new Array(); for (let i = 0; i < this.documents.length; i++) theta[i] = new Array();
    if (this.SAMPLE_LAG > 0) {
  ...
  this.getPhi = function () {
    let phi = new Array(); for (let i = 0; i < this.K; i++) phi[i] = new Array();
    if (this.SAMPLE_LAG > 0) {
      for (var k = 0; k < this.K; k++) {
